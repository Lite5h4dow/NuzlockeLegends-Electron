...
const LoginWithService = (): JSX.Element => {
  ...
  return (
    <Segment loading={state.user.loginService.loading}>
      <Header content="Or sign in with:" floated="left" size="large" />
      <Button circular color="red" basic onClick={loginWithGoogle} icon={"google"} />
      <Button circular color="teal" basic onClick={loginWithTwitter} icon={"twitter"} />
      <Button circular color="black" basic onClick={loginWithApple} icon={"apple"} />
      <Button circular color="grey" basic onClick={LoginWithMicrosoft} icon={"microsoft"} />
    </Segment>
  );
};
