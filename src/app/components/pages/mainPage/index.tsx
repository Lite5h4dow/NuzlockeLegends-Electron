    ...
    <Container>
      <Grid centered>
        ...
        <Grid.Column width="3">
          <Button
            content="Search"
            onClick={() => { getUserHistory(dropdowns.accounts.current!.value.accountId) }}
      ...
      <Divider />
      <Segment>
        {!!!history.data ? <Header textAlign="center" content="No Matches Here" /> : history.data.matches.map((match: any) => <Segment>{match.lane}</Segment>)}
      </Segment>
    </Container>
  );
};
